...
export default function SimuladorKPIs({ outputs }: SimuladorKPIsProps) {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
      {/* Prejuízo Captação */}
      <div
        className={`bg-gradient-to-br ${prejuizoPos ? 'from-indigo-500/15 to-indigo-600/5' : 'from-red-500/15 to-red-600/5'} border ${prejuizoPos ? 'border-indigo-500/30' : 'border-red-500/30'} rounded-xl p-4 transition-all hover:scale-[1.02]`}
      >
        <p className="text-[9px] uppercase tracking-wider text-muted-foreground font-heading mb-1">
          Prejuizo Captacao
        </p>
        <p className={`text-lg font-bold font-mono ${prejuizoPos ? 'text-indigo-400' : 'text-red-400'}`}>
      ...
      {/* TM/CAC */}
      <div className={`bg-gradient-to-br ${outputs.tmCac >= 1 ? 'from-indigo-500/10 to-indigo-600/5 border-indigo-500/20' : 'from-red-500/10 to-red-600/5 border-red-500/20'} border rounded-xl p-4 transition-all hover:scale-[1.02]`}>
        <p className="text-[9px] uppercase tracking-wider text-muted-foreground font-heading mb-1">
          TM / CAC
        </p>
        <p className={`text-lg font-bold font-mono ${outputs.tmCac >= 1 ? 'text-indigo-400' : 'text-red-400'}`}>
